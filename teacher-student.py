...
def nin(): # modify from model - network in network
    ...
    # pre-trained weight
    npyfile = np.load('teacher.npy',allow_pickle=True, encoding='bytes')
    npyfile = npyfile.item()

    weights = {
        'conv1': tf.Variable(npyfile['conv1'][b'weights'], trainable=False, name = 'conv1_w'),
        'cccp1': tf.Variable(npyfile['cccp1'][b'weights'], trainable=False, name = 'cccp1_w'),
        'cccp2': tf.Variable(npyfile['cccp2'][b'weights'], trainable=False, name = 'cccp2_w'),
        'conv2': tf.Variable(npyfile['conv2'][b'weights'], trainable=False, name = 'conv2_w'),
        'cccp3': tf.Variable(npyfile['cccp3'][b'weights'], trainable=False, name = 'cccp3_w'),
        'cccp4': tf.Variable(npyfile['cccp4'][b'weights'], trainable=False, name = 'cccp4_w'),
        'conv3': tf.Variable(npyfile['conv3'][b'weights'], trainable=False, name = 'conv3_w'),
        'cccp5': tf.Variable(npyfile['cccp5'][b'weights'], trainable=False, name = 'cccp5_w'),
        'ip1': tf.Variable(npyfile['ip1'][b'weights'], trainable=False, name = 'ip1_w'),
        'ip2': tf.Variable(npyfile['ip2'][b'weights'], trainable=False, name = 'ip2_w')
    }

    biases = {
        'conv1': tf.Variable(npyfile['conv1'][b'biases'], trainable=False, name = 'conv1_b'),
        'cccp1': tf.Variable(npyfile['cccp1'][b'biases'], trainable=False, name = 'cccp1_b'),
        'cccp2': tf.Variable(npyfile['cccp2'][b'biases'], trainable=False, name = 'cccp2_b'),
        'conv2': tf.Variable(npyfile['conv2'][b'biases'], trainable=False, name = 'conv2_b'),
        'cccp3': tf.Variable(npyfile['cccp3'][b'biases'], trainable=False, name = 'cccp3_b'),
        'cccp4': tf.Variable(npyfile['cccp4'][b'biases'], trainable=False, name = 'cccp4_b'),
        'conv3': tf.Variable(npyfile['conv3'][b'biases'], trainable=False, name = 'conv3_b'),
        'cccp5': tf.Variable(npyfile['cccp5'][b'biases'], trainable=False, name = 'cccp5_b'),
        'ip1': tf.Variable(npyfile['ip1'][b'biases'], trainable=False, name = 'ip1_b'),
        'ip2': tf.Variable(npyfile['ip2'][b'biases'], trainable=False, name = 'ip2_b')
    }
...
def lenet(use_pretrained=False): # modify from lenet model
    ...
    if use_pretrained == False:
        # Random initialize
        weights = {
            'conv1': tf.get_variable('LN_conv1_w', [5,5,3,64],initializer=tf.uniform_unit_scaling_initializer()),
            'conv2': tf.get_variable('LN_conv2_w', [5,5,64,128],initializer=tf.uniform_unit_scaling_initializer()),
            'ip1': tf.get_variable('LN_ip1_w', [5*5*128, 1024] , initializer=tf.uniform_unit_scaling_initializer()),
            'ip2': tf.get_variable('LN_ip2_w', [1024,10], initializer=tf.uniform_unit_scaling_initializer())
        }
...
def read_cifar10(flag):
    ...
    for i in range(len(batches)):
        ...
        for j in range(N):
            data[N*i+j][:,:,2] = np.reshape(temp[b'data'][j][2048:],(32,32))
            data[N*i+j][:,:,1] = np.reshape(temp[b'data'][j][1024:2048],(32,32))
            data[N*i+j][:,:,0] = np.reshape(temp[b'data'][j][:1024],(32,32))
...
def train(model_path, learning_rate, epoch, noisy=False):
    ...
    if noisy == True:
        drop_scale = 1 / Nratio
        noisy_mask = tf.nn.dropout( tf.constant(np.float32(np.ones((batch_size,1)))/drop_scale) ,keep_prob=Nratio) #(batchsize,1)
        gaussian = tf.random_normal(shape=[batch_size,1], mean=0.0, stddev=Nsigma)
        ...
        tf_loss = (lamda*tf.reduce_sum(objective1) + (1 - lamda) * tf.reduce_sum(objective2)) / batch_size
    else:
...
if __name__ == '__main__':
    ...
    with tf.device('/gpu:0'):
        if task=='train':
            train(model, learning_rate=lr, epoch=epoch)
        elif task=='test':
            test_net()
        elif task=='val':
            valid_nin()
